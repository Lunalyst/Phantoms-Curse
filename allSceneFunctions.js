...
class allSceneFunctions{
  ...
  loadGameFile(scene1){
    ...
    console.log("player HP: "+ scene1.healthDisplay);
    ...
    scene1.playerHealth = scene1.healthDisplay;
    ...
    initSlimes(startX, startY, amount, scene,playerSex) {
      ...
      checkSign(scene){
        ...
        scene.signPoints.children.each(function (tempSignPoint){
          ...
          if((scene.player1.x > tempSignPoint.x-50 && scene.player1.x < tempSignPoint.x+50) && (scene.player1.y > tempSignPoint.y-50 && scene.player1.y < tempSignPoint.y+50)&&scene.grabbed === false){
            //console.log("within save point");
          ...
          tempSignPoint.activateSign(scene,scene.keyW,location, scene.activatedSignId,scene.healthDisplay,scene.KeyDisplay,scene.player1,tempSignPoint.x,tempSignPoint.y,scene.flagValues);
    ...
    checkBlueSlimeInteractions(scene){
        ...
        scene.physics.add.overlap(scene.player1, tempSlime,function(){
          ...
          if(tempSlime.grabCoolDown  === 0 && tempSlime.mitosing === false && scene.grabCoolDown === 0){
        ...
        if(tempSlime.DamageCoolDown > 0){
          tempSlime.DamageCoolDown--;
        }
        if(scene.grabCoolDown > 0){
          scene.grabCoolDown--;
        }
