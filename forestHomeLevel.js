...
class forestHomeLevel extends Phaser.Scene {
  ...
  constructor(){
   ...
   //controls the Background
    ...
    that = this;
      setTimeout(function(){
        that.initSlimes(1000, 450, 1);
        that.initSlimes(1000, 550, 1);
    ...
    update(){
      ...
      slimes.children.each(function (tempSlime){
        ...
        //if statement to check if the player and a slime overlap. if so then stops player and calls the grab function
        if(that.checkOverlap(player1,tempSlime) && tempSlime.grabCoolDown === 0){
          tempSlime.setVelocityX(0);
          player1.setVelocityX(0);
          tempSlime.slimeGrab(player1,healthDisplay,keyA,KeyDisplay,keyD,this);
          grabbed = true;
          tempSlime.grabCoolDown = 300;
          console.log('player grabbed by slime');
        }
        if(tempSlime.slimeSize === 1){
          slimes.children.each(function (tempSlime1){
            if(that.checkOverlap(tempSlime1,tempSlime) ){
              console.log("calling combine slimes function")
    ...
    initSlimes(startX, startY, amount) {
      for (let row = 0; row < amount; row++) {
        ...
        var that = this;
          ...
          slime1.slimeId = slimeId;
          slimeId++;
          console.log("slime id: "+ slimeId);
          slimes.add(slime1);
