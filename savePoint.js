...
// lesson learned dont but scene triggers in a overlap function.
    savePointSaveGame(scene1,keyW,location,activeId,hpBar,keyDisplay,player1,saveX,saveY,flagValues){
        //console.log("scene1.safeToSave: "+scene1.safeToSave+" activeId: "+activeId+" this.saveStoneId: "+this.saveStoneId+" this.promptCooldown: "+this.promptCooldown);
        if( this.safeToSave === true && keyW.isDown){
            //console.log("this.nextSceneX "+ this.nextSceneX +" this.nextSceneY: "+this.nextSceneY );
            scene1.activateFunctions.saveGameFile(saveX,saveY,hpBar.playerHealth,scene1.inventoryDataArray,scene1.playerSex,flagValues,scene1.playerLocation);
