...
class playerProjectile extends Phaser.Physics.Arcade.Sprite{
    ...
    constructor(scene, xPos, yPos,type,direction,speedX,speedY,duration,rotation){
      ...
      if(this.type === "sporeCloud"){
      ...
      }else if(this.type === "spindleMissileNectar"){
        ...
        if(temp.destroying === false && (temp.body.blocked.down || temp.body.blocked.left || temp.body.blocked.right || temp.body.blocked.up)){
            ...
            temp.anims.play("spindleMissileDestroy").once('animationcomplete' , () =>{
              temp.collision.destroy();
              temp.colliderRefrenceGround.destroy();
              temp.colliderRefrenceNPC.destroy();
              temp.destroy();
        ...
        scene.npcTriggers.children.each(function (tempNPC) {
          ...
          if(temp.destroying === false && tempNPC.npcType === 'ambush'){
              console.log("projectile has hit the npc!");
              temp.destroying = true;
              temp.isMoving = false;
              temp.setDepth(9);
              temp.scene.initSoundEffect('playerProjectileSFX','missileCrash',0.05);
              ...
              console.log("nectar has been hit now doing recoil function")
              tempNPC.spindleMissileConnected();
          }
